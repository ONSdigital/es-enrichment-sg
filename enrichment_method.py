...
def lambda_handler(event, context):
    ...
    try:
        ...
        enriched_df, anomalies = data_enrichment(input_data, responder_lookup, county_lookup)
...
def marine_mismatch_detector(data, county_lookup_df, county_lookup_column_3, county_lookup_column_4, period_column, identifier_column):
    """
    Detects references that are producing marine but from a county that doesnt produce marine
    :param data - DataFrame: Input data after having been merged with responder_county_lookup
    :param county_lookup_df - DataFrame: County_marine_lookup dataframe
    :param county_lookup_column_3 - String: Column from county lookup to join on (county)
    :param county_lookup_column_4 - String: Column from county lookup representing whether county produces marine or not
    :param period_column - String: Column that holds the period
    :param identifier_column - String: Column that holds the unique id of a row(usually responder id)
    :return: bad_data_with_marine - DataFrame: Df containing information about any reference that is producing marine when it shouldnt
    """
    data_with_marine = pd.merge(data, county_lookup_df, on=county_lookup_column_3)
    bad_data_with_marine = data_with_marine[(data_with_marine['land_or_marine'] == 'M') & (data_with_marine[county_lookup_column_4] == 'n')]
    bad_data_with_marine['issue'] = 'Reference should not produce marine data'
    return bad_data_with_marine[[identifier_column, 'issue', 'land_or_marine', county_lookup_column_4, period_column]]
...
def missing_county_detector(data, county_lookup_column_3, identifier_column):
    ...
    :param data - DataFrame: Input data after being combined with responder_county_lookup
    :param county_lookup_column_3 - String: Column from county lookup to join on (county)
    :param identifier_column - String: Column that holds the unique id of a row(usually responder id)
    :return: data_without_county - DataFrame: DF containing information about any reference without a county.
    """
...
def missing_region_detector(data, county_lookup_column_2, identifier_column):
    ...
    :param data - DataFrame: Input data after being combined with responder_county_lookup and county_marine_lookup
    :param county_lookup_column_2 - String: Column from county lookup that contains region(region)
    :param identifier_column - String: Column that holds the unique id of a row(usually responder id)
    :return: data_without_region - DataFrame: DF containing information about any reference without a region.
    """
...
def data_enrichment(data_df, responder_lookup_df, county_lookup_df):
    ...
    :param responder_lookup_df: Responder lookup DataFrame (map responder code -> county code)
    ...
    responder_lookup_df.rename(columns={"ref": identifier_column, county_lookup_column_3: county_lookup_column_3}, inplace=True)
    county_lookup_df.rename(columns={"cty_code": county_lookup_column_3}, inplace=True)

    enriched_data = pd.merge(data_df, responder_lookup_df, on=identifier_column, how='left')

    anomalies = pd.DataFrame()
    # Do Missing County Check here
    if missing_county_check == 'true':
        anomalies = missing_county_detector(enriched_data, county_lookup_column_3, identifier_column)

    # Do Marine mismatch check here
    if marine_mismatch_check == 'true':
        
        marine_anomalies = marine_mismatch_detector(enriched_data, county_lookup_df, county_lookup_column_3, county_lookup_column_4, period_column, identifier_column)

        anomalies = pd.concat([marine_anomalies, anomalies])

    enriched_data = pd.merge(enriched_data, county_lookup_df[[county_lookup_column_1, county_lookup_column_2, county_lookup_column_3]], on=county_lookup_column_3, how='left')

    if missing_region_check == 'true':

        region_anomalies = missing_region_detector(enriched_data, county_lookup_column_2, identifier_column)
