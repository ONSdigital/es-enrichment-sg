...
class test_enrichment(unittest.TestCase):
    ...
    def test_catch_exception(self):
        ...
        with mock.patch.dict(lambda_wrangler_function.os.environ, {
            ...
            with mock.patch('enrichment_wrangler.get_from_s3') as mocked:
                ...
                response = lambda_wrangler_function.lambda_handler({"RuntimeVariables":{"checkpoint":666}}, None)
    ...
    def test_catch_method_exception(self):
        ...
        with mock.patch.dict(lambda_wrangler_function.os.environ, {
            ...
            with mock.patch('enrichment_wrangler.boto3.resource') as mocked:
                mocked.side_effect = Exception('SQS Failure')
                response = lambda_method_function.lambda_handler({"RuntimeVariables":{"checkpoint":666}}, None)
                assert 'success' in response
                assert response['success'] is False
    ...
    def test_missing_county_detector(self):
        ...
        test_output = lambda_method_function.missing_county_detector(data,"county","responder_id")
        assert test_output.shape[0] == 1

    def test_missing_region_detector(self):
        data = pd.DataFrame({"region": [1, None, 2], "responder_id": [666, 123, 8008]})
        test_output = lambda_method_function.missing_region_detector(data, "region", "responder_id")
        assert test_output.shape[0] == 1
    ...
    def test_marine_mismatch_detector(self):
        ...
        testdata_df = pd.merge(testdata_df, responder_lookup_df, on="responder_id", how='left')

        test_output = lambda_method_function.marine_mismatch_detector(testdata_df,countylookupdata_df,"county", "marine", "period", "responder_id")
        assert test_output.shape[0] == 1
    ...
    def test_data_enricher(self):
        with mock.patch.dict(lambda_method_function.os.environ, {
            ...
            countylookupdata_df = pd.DataFrame(json.loads(countylookupdata))
            responder_lookup_df = pd.DataFrame(json.loads(responder_lookup))
            test_output,test_anomalies = lambda_method_function.data_enrichment(testdata_df,responder_lookup_df, countylookupdata_df, "responder_id",
                                                                                'county_name', 'region', 'county', 'marine', 'true', 'true', 'true', 'period')
