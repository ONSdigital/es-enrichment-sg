...
class TestEnrichment(unittest.TestCase):
    ...
    def test_missing_county_detector(self):
        data = pd.DataFrame({"county": [1, None, 2],
                           "responder_id": [666, 123, 8008]})
        test_output = lambda_method_function.missing_county_detector(data,
                                                                     "county",
                                                                     "responder_id")
    ...
    def test_missing_region_detector(self):
        ...
        test_output = lambda_method_function.missing_region_detector(data,
                                                                     "region",
                                                                     "responder_id")
        assert test_output.shape[0] == 1
    ...
    def test_marine_mismatch_detector(self):
        ...
        testdata_df = pd.merge(testdata_df, responder_lookup_df,
                               on="responder_id", how='left')

        test_output = lambda_method_function.marine_mismatch_detector(testdata_df,
                                                                      countylookupdata_df,
                                                                      "county",
                                                                      "marine",
                                                                      "period",
    ...
    def test_data_enricher(self):
        with mock.patch.dict(lambda_method_function.os.environ, {
            ...
            countylookupdata_df = pd.DataFrame(json.loads(countylookupdata))
            responder_lookup_df = pd.DataFrame(json.loads(responder_lookup))
            test_output, test_anomalies = lambda_method_function.data_enrichment(testdata_df,
                                                                                responder_lookup_df,
                                                                                countylookupdata_df)
